...
function rename() { //open the rename panel
	...
	nameTitle.text('Renaming ' + renameOrig).append(escHelp); //display what is being renamed
	nameInput.keydown(); //trigger the code that checks to see if the new name is valid (make sure no other key is named '')
	namePanel.show(); //open the renaming window
	nameInput.focus(); //target the name input for text input
}
function checkName() { //checks the value of the nameinput to see if it conflicts
	var value = $(this).val(); //get value
	if (value == renameOrig) nameSaveButton.removeClass('btn-success').removeClass('btn-danger'); //if the changed value is the same as the original one, show that nothing will be changed
		...
		if (value.length < 32768) { //make sure length fixed in a short
			...
			if (success) nameSaveButton.removeClass('btn-danger').addClass('btn-success'); //if it is legal, show so
			else nameSaveButton.removeClass('btn-success').addClass('btn-danger'); //otherwise show that it isn't allowed
		}
		else nameSaveButton.removeClass('btn-success').addClass('btn-danger');
	}
}
function saveName() { //save the new name
	if (nameSaveButton.hasClass('btn-success')) { //if the name is invalid, do nothing
		const path = getPath(saveTag); //get the path before renaming
		const newName = nameInput.val(); //fetch the new name
		...
		if (saveTag.children('img.type').attr('src') == IMAGES.TAG_List || saveTag.children('img.type').attr('src') == IMAGES.TAG_Compound || saveTag.children('img.type').attr('src') == IMAGES.TAG_Byte_Array || saveTag.children('img.type').attr('src') == IMAGES.TAG_Int_Array) spanChild.text(newName + ':'); //if the tag shouldn't display its value, just display the new name
