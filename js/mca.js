...
MCARead.prototype.getChunkTimestamp = function(x, z) {
...
MCARead.prototype.getChunkData = function(x, z) {
	...
	switch (compressionScheme) {
		case GZIP_COMPRESSION:
			return new Buffer(jz.gz.decompress(compressedChunk.rawBuffer()).buffer);
		case ZLIB_COMPRESSION:
			return new Buffer(jz.zlib.decompress(compressedChunk.rawBuffer()).buffer);
		default:
			throw new Error('Unknown compression scheme: ' + String(compressionScheme));
	}
};
//Read the entire region into chunks
MCARead.prototype.getAllChunks = function() {
	var result = [];
...
MCAWrite.prototype.setChunkTimestamp = function(x, z, timestamp) {
...
MCAWrite.prototype.setChunkLocation = function(x, z, sector, length) {
	const offset = calculateChunkOffset(x, z);
...
MCAWrite.prototype.setAllChunks = function(data) {
	if (this.buffer.length != SECTOR_LENGTH * 2) this.buffer = new MCAWrite().getBuffer(); //make sure no chunks are already stored
...
MCAWrite.prototype.getBuffer = function() {
	return this.buffer;
};